...
@section('content')
    <div class="space-y-8">
        ...
        <!-- Registration Form -->
        <form action="{{ route('user.register.phone.submit') }}" method="POST" class="space-y-6 max-w-md mx-auto">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <div class="relative flex1 group">
                    <input type="text" name="name" id="name" required value="{{ old('name') }}"
                        class="w-full p-4 pl-12 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-green-500 focus:ring-4 focus:ring-green-200
            ...
            <div class="space-y-2">
                ...
                <div class="flex group">
                    <div
                        class="flex items-center bg-gray-100 border-2 border-r-0 border-gray-200 text-gray-500 px-4 rounded-l-xl">
                        +62
                    </div>
                    <div class="relative flex-1">
                        <input type="tel" id="phone_input" required
                            class="w-full p-4 pl-12 border-2 border-gray-200 rounded-r-xl focus:outline-none focus:border-green-500 focus:ring-4 focus:ring-green-200
                                  transition-all duration-300 placeholder-gray-400 peer"
                            placeholder="8123456789" pattern="[0-9]*" inputmode="numeric"
                            value="{{ old('phone_number') ? substr(old('phone_number'), 3) : '' }}">
                        <svg class="w-6 h-6 absolute left-4 top-4 text-gray-400 transition-colors duration-300 group-focus-within:text-green-500"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                        </svg>
            ...
            <!-- Submit Button -->
            <button type="submit"
                class="w-full py-4 bg-gradient-to-r from-green-500 to-green-600 text-white font-bold rounded-xl
                   hover:from-green-600 hover:to-green-700 hover:shadow-lg transform transition-all
        ...
        <div class="space-y-4 max-w-md mx-auto">
            <a href="{{ route('user.register.email') }}"
                class="w-full p-4 bg-blue-500 text-white rounded-xl flex items-center justify-center space-x-2
            ...
            <p class="text-center text-gray-500">
                Sudah punya akun?
                <a href="{{ route('user.login.phone') }}" class="text-green-500 hover:underline font-medium">Masuk
                    Sekarang</a>
            </p>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const fullPhoneField = document.getElementById('full_phone_number');

            // Format nomor telepon saat form disubmit
            document.querySelector('form').addEventListener('submit', function(e) {
                fullPhoneField.value = phoneInput.value;
            });
        });
